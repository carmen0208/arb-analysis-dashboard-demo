...
/**
 * Calculate tick swap capacity for a Uniswap V3 pool
 *
 * This function calculates the maximum swap amounts that can be made before
 * crossing the next/previous ticks, along with price impact analysis.
 *
 * @param poolAddress - The pool address to analyze
 * @param config - Pool configuration
 * @param includeUSD - Whether to include USD calculations (requires token price data)
 * @returns TickSwapCapacity object with up/down/current information
 */
export async function getTickSwapCapacity(
  poolAddress: `0x${string}`,
  config?: V3PoolConfig,
  includeUSD: boolean = false,
): Promise<TickSwapCapacity> {
  try {
    // Get pool base info with token information
    const poolBaseInfo = await getPoolBaseInfoWithTokenInfo(poolAddress);
    const {
      currentTick,
      tickSpacing: tickSpacingNum,
      token0: token0Info,
    ...
    // Get tick liquidity distribution to find next/previous ticks
    const tickDistribution = await getTickLiquidityDistribution(
      poolAddress,
      5, // Smaller range for efficiency
    );

    // Find current tick index
    const currentTickIndex = tickDistribution.findIndex(
      (tick) => tick.currentTick,
    );

    if (currentTickIndex === -1) {
      throw new Error("Current tick not found in distribution");
    }

    // Get next and previous ticks
    const nextTick = tickDistribution[currentTickIndex + 1];
    const prevTick = tickDistribution[currentTickIndex - 1];

    // Calculate current tick info
    const sqrtRatioCurrentX96 = TickMath.getSqrtRatioAtTick(currentTick);
    const currentRatio = Math.pow(Number(sqrtRatioCurrentX96) / Number(Q96), 2);

    // Determine liquidity distribution direction
    const liquidityDistribution: "token0" | "token1" =
      currentTick < 0 ? "token1" : "token0";

    // Calculate ratio impact (price change percentage)
    const ratioImpact = 0; // Current tick has no impact

    const current: CurrentTickInfo = {
      currentTick,
      currentRatio,
      sqrtRatioCurrent: Number(sqrtRatioCurrentX96),
      liquidityDistribution,
      ratioImpact,
    };

    // Calculate up direction (next tick)
    let up: SwapAmountInfo;
    if (nextTick) {
      const sqrtRatioNextX96 = TickMath.getSqrtRatioAtTick(nextTick.tick);
      const nextRatio = Math.pow(Number(sqrtRatioNextX96) / Number(Q96), 2);
      const ratioImpactUp = ((nextRatio - currentRatio) / currentRatio) * 100;

      // Calculate max swap amounts to reach next tick
      const maxSwapAmounts = calculateTokenAmounts(
        nextTick.tick,
        nextTick.liquidityNet,
        currentTick,
        token0Info,
        token1Info,
        nextTick.availableLiquidity,
      );

      up = {
        token0AmountAdjusted: maxSwapAmounts.token0AmountAdjusted,
        token1AmountAdjusted: maxSwapAmounts.token1AmountAdjusted,
        token0TotalUSDAdjusted: includeUSD
          ? maxSwapAmounts.token0TotalUSDAdjusted
          : undefined,
        token1TotalUSDAdjusted: includeUSD
          ? maxSwapAmounts.token1TotalUSDAdjusted
          : undefined,
        nextTick: nextTick.tick,
        nextRatio,
        ratioImpact: ratioImpactUp,
        sqrtRatioNext: sqrtRatioNextX96.toString(),
        direction: "ratio_up",
      };
    } else {
      // No next tick found, use default values
      up = {
        token0AmountAdjusted: 0,
        token1AmountAdjusted: 0,
        token0TotalUSDAdjusted: includeUSD ? 0 : undefined,
        token1TotalUSDAdjusted: includeUSD ? 0 : undefined,
        nextTick: currentTick + tickSpacingNum,
        nextRatio: currentRatio,
        ratioImpact: 0,
        sqrtRatioNext: TickMath.getSqrtRatioAtTick(
          currentTick + tickSpacingNum,
        ).toString(),
        direction: "ratio_up",
      };
    }

    // Calculate down direction (previous tick)
    let down: SwapAmountInfoDown;
    if (prevTick) {
      const sqrtRatioPrevX96 = TickMath.getSqrtRatioAtTick(prevTick.tick);
      const prevRatio = Math.pow(Number(sqrtRatioPrevX96) / Number(Q96), 2);
      const ratioImpactDown = ((prevRatio - currentRatio) / currentRatio) * 100;

      // Calculate max swap amounts to reach previous tick
      const maxSwapAmounts = calculateTokenAmounts(
        prevTick.tick,
        prevTick.liquidityNet,
        currentTick,
        token0Info,
        token1Info,
        prevTick.availableLiquidity,
      );
      ...
      down = {
        ...
        token1TotalUSDAdjusted: includeUSD
          ? maxSwapAmounts.token1TotalUSDAdjusted
          : undefined,
        prevTick: prevTick.tick,
        prevRatio,
        ratioImpact: ratioImpactDown,
        sqrtRatioPrev: sqrtRatioPrevX96.toString(),
        direction: "ratio_down",
      };
    } else {
      // No previous tick found, use default values
      down = {
        token0AmountAdjusted: 0,
        token1AmountAdjusted: 0,
        token0TotalUSDAdjusted: includeUSD ? 0 : undefined,
        token1TotalUSDAdjusted: includeUSD ? 0 : undefined,
        prevTick: currentTick - tickSpacingNum,
        prevRatio: currentRatio,
        ratioImpact: 0,
        sqrtRatioPrev: TickMath.getSqrtRatioAtTick(
          currentTick - tickSpacingNum,
        ).toString(),
        direction: "ratio_down",
      };
    ...
    logger.error("[getTickSwapCapacity] Error calculating tick swap capacity", {
      poolAddress,
      error: error instanceof Error ? error.message : String(error),
    });
    throw error;
